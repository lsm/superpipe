...
| `__proto__` inert setter on container/producer writes (`setEntry` uses `defineProperty`, an own key — common.ts:128-138) — but the fetcher's object form assigns plainly (`result[key] = …`, Fetcher.ts:208-215), so `.InFields("__proto__")` / `OutputFields("__proto__")` **silently drop the field** (or retarget the result's prototype when the value is an object) instead of delivering it | Plain `map[string]any` assignment **everywhere**: `__proto__` is an ordinary own key on both the write and the fetch side, and `.InFields("__proto__")` / `OutputFields("__proto__")` deliver the bound value normally | Go maps have no prototype chain; the attack surface does not exist. Matching the fetch-side drop would import JS's object model to reproduce a hazard, so the port keeps the write-side (designed) behavior on both paths |
| Property reads execute **accessors** wherever the source is a JS object: a configured dep (`Fetcher.lookup`'s `functions[key]`), a picked/destructured key on a step's returned object (Producer.ts:160-166), every enumerable value a `Merge()` reads (executor.ts:109-120), and an `InputFromObject` source member (Producer.ts:228-233) — each read can run user code, have side effects, and **throw** (a throwing `out` getter fails the final-output fetch; test *rejects when the settled output lookup throws*) | Every read is a plain map/index access: no Go read executes user code, has side effects, or throws | Go maps and slices have no accessors or proxy traps. Computed values are spelled the way Go always spells them — computed before `Run` or inside a step. The JS behavior is an artifact of every data source being an arbitrary JS object |
...
| **Own** data properties on non-map sources also bind: a class instance with an own `tag` field, or an array with an own string property (`source.tag = 'x'`), deliver through `InputFromObject` — `produceFromObject` reads `source[key]` on any source kind (Producer.ts:228-233) | A struct field or an array-attached property is **not** readable: named reads see map keys, and on slices/arrays/strings only canonical indexes and `length`; everything else binds nil (§3.2) | A Go slice cannot carry own properties at all — unrepresentable, not declined. A struct's fields are typed, not string-keyed entries; reflecting them into the string-keyed container would blur the map/struct boundary the rest of the spec keeps sharp — put named data in a map, or bind the whole value with `Out` |
...
| A step declaring `next` could advance **explicitly valueless** — bare `next()` bypasses `expectValue`, which guards only direct returns (executor.ts:392-397; test *still allows a next-based pipe to advance without a value*) | No valueless-advance spelling exists under a value-requiring spec: `(nil, nil)` fails the §3.2 value requirement with `OutputKeyError` (C5) | Bare `next()` was the callback channel's explicit nothing-to-merge signal; with `next` removed a nil return is indistinguishable from a step that forgot to produce — exactly what the requirement catches. A skipped `Optional` step still advances valuelessly (C7) |
...
| `Object.keys` preserves **insertion order** for string keys — a `Merge()` of `{z:1, a:2}` validates `z` first | String-key `Merge()` validation is **lexicographic** (`a` before `z`); array-index keys still order numerically (C4) | A Go map cannot carry insertion order; lexicographic is the deterministic stand-in, observable only in which of several invalid keys is reported |
...
| Error handler return ignored; a *throwing* handler reaches the `end()` caller only when a **synchronous** failure invokes it before `end()` returns — a failure delivered later (promise rejection, retained callback) throws after `end()` has returned, on that asynchronous stack; under `endAsync` the run settles with the active error *before* the handler runs and the later throw is discarded by the already-settled guard (executor.ts:428-434, 499-503) | Handler returns `error`; non-nil returns and recovered panics join the settlement error via `errors.Join` | One error channel — swallowing handler failures (DLQ writes, alerting) would be un-Go; the TS async swallow is a settlement-ordering artifact, and the port's uniform behavior matches the synchronous-failure path, which is the designed contract |
...
- Invocation-input specs (the `Input` **def** — not a step's `.In`, which declares
  ...
  **non-nil** string-keyed map and not a non-nil indexed collection — a scalar,
  struct, pointer (typed-nil pointers included), a nil map or slice (a nil source,
  per the collection rule above), or any other non-map, non-indexed kind — binds
  every requested name present-with-nil, mirroring JS property access on primitives;
  no source kind is an error. **Non-nil slices, arrays, and strings are excluded from
  that fallback**: they are indexed collections, governed by the numeric-index and
  `length` rules that follow — a canonical index binds the element at that position,
  a non-canonical key binds nil, and `length` binds the collection's length — never
  the blanket nil above. (A **nil slice is a nil source**, so every requested name,
  `length` included, binds present-with-nil; an **empty non-nil** slice or string is
  a present indexed collection — `length` binds 0, mirroring `[].length` and
  `"".length` in JS.)
...
- Step inputs have an object form: `.InFields("error", "key1")` resolves each name per
  ...
  is most common. A member named `__proto__` is an **ordinary own key** here and in
  `OutputFields` — the reference's `fetchAsObject` assigns plainly and silently drops
  it (Fetcher.ts:208-215); the port delivers it, per §2's `__proto__` row.
...
- The return is the single continuation channel:
  ...
  **reflect-based**, not `value != nil` on the interface: a **typed nil** (a nil
  ...
  executor.ts:394-397) — a deliberate divergence, §2: a TS step declaring `next`
  could advance explicitly valueless (bare `next()` bypasses `expectValue`, which
  guards only direct returns — executor.ts:392-397, test *still allows a next-based
  pipe to advance without a value*); the callback-free port has no such channel, and
  `(nil, nil)` is exactly the forgotten-produce return the requirement rejects.
  `(nil, err)` → error path — and a **typed-nil** value returned
...
| `flow_control_test.go` | `flow-control-contract.test.mjs` (~2,700 lines) | the whole of §4; the acceptance bar. Tests whose premise is unportable — accessor-backed dep lookups (§2), prototype-inherited deps (§2), `next`-channel misuse (decision 4) — are replaced by their §2/§4 counterparts or dropped with a comment naming the divergence |
