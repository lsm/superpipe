...
| A `!`-marked dep delivering its boolean through `next(null, true)` is **not** inverted or flow-controlled — the callback wrapper advances directly (Fetcher.ts:47-64; inversion covers only direct/thenable results, executor.ts) | A boolean-resolved step's return is **always** inverted and flow-controlled, whatever delivery style the original TS used | The non-inversion is a callback-path artifact; with `next` removed (decision 4) there is no second delivery path to exempt |
