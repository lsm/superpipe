...
| A configured dep may be an **accessor**: `Fetcher.lookup`'s `functions[key]` executes the getter, so a lookup can run user code, have side effects, and **throw** — a throwing `out` getter makes the final-output fetch fail the run (test *rejects when the settled output lookup throws*) | `Deps` is a plain map: a lookup never executes user code and cannot fail | Go maps have no accessors or proxy traps; computed dependencies are spelled the way Go always spells them — computed before `Run`, or inside a step. The JS behavior is an artifact of `functions` being an arbitrary JS object |
