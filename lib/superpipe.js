...
var SuperPipe = module.exports = function SuperPipe(injector) {
  ...
  this.reservedDeps = ['autoBind', 'emit',
    'setDep', 'getDep', 'set', 'get',
    'next'
  ];

  var fns = ['autoBind', 'isDepNameReserved', 'emit']
...
SuperPipe.prototype.isDepNameReserved = function(name) {
  if ('string' === typeof name && indexOf(this.reservedDeps, name) > -1) {
    throw new Error('The name of your dependency is reserved: ' + name)
  } else if (isArray(name)) {
    forEach(this.reservedDeps, function(reserved) {
...
SuperPipe.prototype.setDep = function(name, dep, props) {
  var normalized = normalizeArgs(name, dep, props)
  ...
  if (props && 'object' === typeof dep) {
    ...
    forEach(props, function(propName) {
      ...
      emitChange(self, _name, depObject, injector)
      injector.regDependency(_name, depObject)
    })
  } else if (name) {
    // setDep(name, value)
    emitChange(self, name, dep, injector)
...
SuperPipe.prototype.onSetDep = function(depName, callback) {
  var eventName = '__change__' + depName
...
SuperPipe.prototype.normalizeSetDepArgs = normalizeArgs

/**
 * Private functions
 */

function normalizeArgs(name, dep, props) {
...
function emitChange(emitter, depName, newValue, injector) {
  if (emitter && emitter.listeners) {
    var eventName = '__change__' + depName
    if (emitter.listeners(eventName).length > -1) {
      emitter.emit(eventName, newValue, injector.getDependency(depName))
    }
  }
}
