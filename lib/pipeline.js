      ...
      var next = function(err) {
        ...
        if (pipe) {
          ...
          if (modifySetDep) {
            // Modify the `setDep`
            var fulfilled = false
            var fulfillCounter = autoNext ? 0 : false
            ...
            injector.regDependency('setDep', function(name, dep, props) {
              ...
              fulfilled = _props.every(function(propName) {
                return -1 !== supplies.indexof(propName)
              })

              if (!fulfilled) {
                return next('Dependency is not defined in supplies.')
              }

              // Count supplies we get.
              if (fulfillCounter !== false) {
                fulfillCounter += _props.length
              }

              // Set the dependencies.
              setDep(name, dep, props)

              // We should call next when all required supplies are fulfilled.
              if (fulfillCounter === supplies.length && !err && result !== false) {
                next()
              } else if (fulfillCounter > supplies.length) {
          ...
          if (modifySetDep) {
            ...
            injector.regDependency(setDep)
          }

          // check if we need to run next automatically
          if (!err && result !== false && autoNext) {
