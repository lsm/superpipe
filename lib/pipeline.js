...
pipelineFunctions.dipe = function(name, deps, supplies) {
  ...
  if ('set' === name) {
    // a `set` pipe function with customized position based supplies
    var _supplies = pipe.supplies
    if (_supplies && (!deps || 'number' === typeof deps || isArray(deps))) {
      pipe.fn = function() {
        var args
        var positioned
        ...
        var result = {}
        foreach(_supplies, function(supplyName, idx) {
          result[supplyName] = positioned ? args[idx] : args[supplyName]
        })
        return result
      }
    } else if ('object' === typeof deps && !isArray(deps)) {
      // mapped dependencies by position of supplies
      pipe.deps = null
      pipe.fn = function() {
        var args = arguments
        var result = {}
...
pipelineFunctions.toPipe = function(superpipe, name) {
  if ('string' === typeof superpipe) {
    name = superpipe
    superpipe = null
  }
  var cloned = this.clone(superpipe)
  return Pipeline.toPipe(cloned, name || this.Name())
}
...
pipelineFunctions.toCurriedPipe = function(name) {
  ...
  return function(superpipe) {
    ...
    return Pipeline.toPipe(sliced, name || this.Name())
  }
}
