...
exports.toPipe = function(pipeline, pipes, superpipe) {
  ...
  if (debug)
    pipes = addDebugPipes(pipes, 'function' === typeof debug ? debug : null)
    ...
    if (pipes[0]) {
      ...
      if (superpipe) {
        ...
        injector.set('set', function(name, dep, props) {
          DEP.setDep(superpipe, _injector, name, dep, props)
...
function addDebugPipes(pipes, debug) {
  var _pipes = []
  debug = debug || log
  foreach(pipes, function(pipe, idx) {
    ...
    var beforePipe = {
      fn: function() {
        ...
        debug('\nExecuting pipeline [%d]: [%s] \n\t[deps expect]: %s\t[deps actual]: %s\t[sups expect]: %s'
          , idx, fnName,
